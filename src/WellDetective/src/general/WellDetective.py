...
class WellDetective:
    ...
    def plot_corrected_mag_by_heading(self, markevery=10, figsize=None, save_path=None):
        """
        ...
        markevery : int, optional
            Plot every Nth point to reduce density (default: 10). This parameter
            controls data subsampling. Setting markevery=10 plots every 10th 
            data point. If None or 0, plots all data.
        figsize : tuple, optional
        ...
        Example
        -------
        >>> wd = WellDetective.Load_Process_MagDataFile('path/to/file.csv')
        >>> # Plot every 10th point
        >>> fig, axes = wd.plot_corrected_mag_by_heading(markevery=10)
        >>> plt.show()
        """
        from matplotlib.collections import LineCollection
        
        # Check required columns
        if self.data_filtered is None or self.data_filtered.empty:
            raise ValueError("data_filtered is empty. Process data first.")
        if 'Corrected' not in self.data_filtered.columns:
        ...
        for idx, filename in enumerate(files):
            ...
            # Subsample data using markevery
            if markevery is not None and markevery > 0:
                plot_data_sub = plot_data.iloc[::markevery].copy()
            else:
                plot_data_sub = plot_data.copy()
            
            # Get data
            mag_corr = plot_data_sub['Corrected'].values
            heading = plot_data_sub['Heading'].values
            ...
            # Add statistics as text (use full dataset stats, not subsampled)
            mean_mag = plot_data['Corrected'].mean()
            std_mag = plot_data['Corrected'].std()
            n_points = len(plot_data)
            n_plotted = len(plot_data_sub)
            subsample_info = f'every {markevery}' if markevery else 'all'
            stats_text = f'n={n_points:,} (plotting {subsample_info}) | μ={mean_mag:.1f} nT | σ={std_mag:.1f} nT'
            ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, 
                    fontsize=9, verticalalignment='top',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
        
        # Bottom plot gets x-label
        axes[-1].set_xlabel('Sample Index (subsampled)', fontsize=10)
